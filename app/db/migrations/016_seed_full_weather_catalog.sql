...
-- ── Cleanup old labels before re-seeding (ensures no duplicates in existing DBs) ──
DELETE FROM data_units WHERE data_type_id = 5 AND unit IN ('fraction', '%');
DELETE FROM data_units WHERE data_type_id = 7 AND unit = 'unitless';
DELETE FROM data_units WHERE data_type_id = 8 AND unit = 'unitless';
DELETE FROM data_units WHERE data_type_id = 9 AND unit IN ('umol/mol', 'kg/m^3');

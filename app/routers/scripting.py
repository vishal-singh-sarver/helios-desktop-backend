...
async def execute_script(req: ScriptExecuteRequest):
    ...
    async def _run():
        ...
        try:
            async with asyncio.timeout(req.timeout):
                with redirect_stdout(stdout_buf), redirect_stderr(stderr_buf):
                    await asyncio.to_thread(exec, req.code, namespace)
        except TimeoutError:
            error = f"Script timed out after {req.timeout}s"
        except Exception as ex:
            error = traceback.format_exc()

        duration = time.time() - t0

        # Detect new and deleted primitives
        ...
        if new_uuids:
            ...
            # Group by material for children
            children = []
            for mat in new_mats:
                if mat.startswith("__auto_"):
                    # Promote auto material to named
            ...
            new_object = {"object_id": obj_id, "name": reg.get_object(obj_id)["name"],
                          "uuids": new_uuids}
